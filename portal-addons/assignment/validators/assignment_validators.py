    ...
    def decorator(origin_function): 
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                logger.warn("Some one has submitted with an invalid access token")
...
def check_match_student():
    def decorator(origin_function): 
        def wrapper(self, *args, **kwargs):
            payload_student_id = json.loads(request.httprequest.data)["student_id"]
            access_token_student_id = self.student.id

            if self.student.id != json.loads(request.httprequest.data)["student_id"]:
                logger.warn(f"SUSPICIOUS BEHAVIOR: student with id {access_token_student_id} has submitted a submission for a student with id {payload_student_id}")
                return json_response(403, "Forbidden")
